...
String msgArray[5]={"CALL IN PROGRESS ","DO NOT DISTURB ","GONE FOR A RUN ", "VERY BUSY ", "SLEEPING "}; // preset messasges - dont forget to change msgMax if you add more.
int msgCounter=0;
int buttonPress=0;
int msgMax=5;
...
{
  static enum { S_IDLE, S_WAIT_CONN, S_READ, S_EXTRACT, S_RESPONSE, S_DISCONN, B_PRESSED } state = S_IDLE;
  ...
  switch (state)
  {
    ...
    case S_WAIT_CONN:   // waiting for connection
      {
        ...
        if (digitalRead(D2) == LOW) // added for button
        {
          buttonPress=1;
          state=B_PRESSED;
          break;
    ...
    case B_PRESSED: // this added for when a botton is pressed
      state = S_IDLE;
      if (msgCounter<msgMax){
          for (int i=0; i<msgArray[msgCounter].length(); i++) { // move the string message into the char array
            newMessage[i]=msgArray[msgCounter].charAt(i);
          }
    
          for (int i=msgArray[msgCounter].length(); i<255; i++) { // ensure rest of char array empty
            newMessage[i]=0;
          }
      }
      else {
        for (int i=0; i<255; i++) { // ensure char array empty
        newMessage[i]=0;
         }
        // if cycled round all messages then use the IP address info again before restarting back from first message
      ...
      delay(500); //short delay to ensure button depressed
      break;
